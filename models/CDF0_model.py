...
class CDF0Model(BaseModel):
    ...
    def forward(self):
        ...
        self.feat_AB1,self.feat_AB2 = self.netF(self.A,self.B)  # f(A)
        # self.feat_B = self.netF(self.B)   # f(B)

        self.dist1 = self.feat_AB1
        self.dist2 = self.feat_AB2
        # print(self.dist.shape)
        self.dist1 = F.interpolate(self.dist1, size=self.A.shape[2:], mode='bilinear',align_corners=True)
        self.dist2 = F.interpolate(self.dist2, size=self.A.shape[2:], mode='bilinear',align_corners=True)

        self.pred_L = (self.dist2 > 1).float()
    ...
    def backward(self):
        ...
        # print(self.weight)
        self.loss_f1 = self.criterionF(self.dist1, self.L_s)
        self.loss_f2 = self.criterionF(self.dist2, self.L_s)
        # self.loss_f = self.loss_f2+self.loss_f1
        # self.loss_f = 0.8*self.loss_f2+0.2*self.loss_f1
        # self.loss_f = 0.7*self.loss_f2+0.3*self.loss_f1
        self.loss_f = 0.6*self.loss_f2+0.4*self.loss_f1
        # self.loss_f = 0.5*self.loss_f2+0.5*self.loss_f1
